...
function findNearestAuctionVendor(){
  try{
    ...
    let best = null, bestSteps = Infinity;
    for(const k in AUTO_AUCTION_VENDORS){
      const v = AUTO_AUCTION_VENDORS[k];
      if(norm(v.map) === cur) return v;
      const s = eqStepsToMap(v.map);
      if(s < bestSteps){ bestSteps = s; best = v; }
    }
    return best || AUTO_AUCTION_VENDORS['torneg-aukcjoner'] || Object.values(AUTO_AUCTION_VENDORS)[0];
  }catch(_){ return AUTO_AUCTION_VENDORS['torneg-aukcjoner'] || Object.values(AUTO_AUCTION_VENDORS)[0]; }
}
...
// ===== AUTO AUCTION WALKER (free slots threshold -> nearest Aukcjoner by graph) =====
(function(){
  const CHECK_MS = 1200;
  const START_COOLDOWN_MS = 15000;
  let lastStartAt = 0;

  function adiHasAuctionItemsToList(){
    try{ return !!eqFindAuctionItemInBag(); }catch(_){ return true; }
  }
  ...
  function adiStartAuctionWalk(reason){
    try{
      ...
      setTempTarget(v.map);
      startEquipFlow();
      eqSetInfo('Mało miejsca w torbie -> idę do najbliższego Aukcjonera: ' + v.map + ' (' + v.stand.x + ',' + v.stand.y + ').', true);
      const btn=document.querySelector('#adi-bot_toggle'); if(btn && btn.innerText==='START') btn.click();
      return true;
    }catch(_){ return false; }
  }
  ...
  setInterval(()=>{
    try{
      ...
      const free = Number(bagSpace.free);
      if(free > threshold) return;
      if(!adiHasAuctionItemsToList()) return;

      adiStartAuctionWalk('free<=' + threshold + '|free=' + free);
    }catch(_){ }
  }, CHECK_MS);
})();
