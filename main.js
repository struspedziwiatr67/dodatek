...
try{
  ...
  tabTest.appendChild(exh530Row);
      ...
      try{
        ...
        ulTitle.textContent = 'Ulepszanie ekwipunku';
        ulTitle.style.fontWeight = '700';
        ulTitle.style.marginBottom = '8px';
        ulWrap.appendChild(ulTitle);

        const ulInfo = document.createElement('div');
        ulInfo.style.fontSize = '12px';
        ulInfo.style.marginBottom = '8px';
        ulInfo.style.opacity = '0.85';
        ulInfo.textContent = 'Tutaj możesz skonfigurować ulepszanie (zaklęcia, kamienie, kowalstwo).';
        ulWrap.appendChild(ulInfo);

        // Sekcja: typ ulepszenia
        const ulTypeLabel = document.createElement('label');
        ulTypeLabel.style.display = 'block';
        ulTypeLabel.style.marginBottom = '4px';
        ulTypeLabel.textContent = 'Typ ulepszenia:';
        ulWrap.appendChild(ulTypeLabel);

        const ulTypeSel = document.createElement('select');
        ulTypeSel.id = 'adi-bot_upgrade_type';
        ulTypeSel.className = 'adi-bot_inputs';
        ulTypeSel.style.marginBottom = '6px';
        [['none','Brak (wyłączone)'],['zaklecie','Zaklęcie'],['kamien','Kamień szlif.'],['kowalstwo','Kowalstwo']].forEach(([val, txt])=>{
          const o = document.createElement('option'); o.value=val; o.textContent=txt; ulTypeSel.appendChild(o);
        });
        try{ ulTypeSel.value = localStorage.getItem('adi-bot_upgrade_type') || 'none'; }catch(_){}
        ulTypeSel.addEventListener('change', ()=>{ try{ localStorage.setItem('adi-bot_upgrade_type', ulTypeSel.value); message('Ulepszanie: typ zapisany – ' + ulTypeSel.value); }catch(_){} });
        ulWrap.appendChild(ulTypeSel);

        // Slot ulepszenia
        const ulSlotLabel = document.createElement('label');
        ulSlotLabel.style.display = 'block';
        ulSlotLabel.style.marginBottom = '4px';
        ulSlotLabel.textContent = 'Slot przedmiotu (nazwa):';
        ulWrap.appendChild(ulSlotLabel);

        const ulSlotInput = document.createElement('input');
        ulSlotInput.type = 'text';
        ulSlotInput.id = 'adi-bot_upgrade_item';
        ulSlotInput.className = 'adi-bot_inputs';
        ulSlotInput.placeholder = 'np. Stalowy miecz';
        ulSlotInput.style.marginBottom = '6px';
        ulSlotInput.setAttribute('tip', 'Nazwa przedmiotu do ulepszenia (fragment nazwy)');
        try{ ulSlotInput.value = localStorage.getItem('adi-bot_upgrade_item') || ''; }catch(_){}
        ulSlotInput.addEventListener('keyup', ()=>{ try{ localStorage.setItem('adi-bot_upgrade_item', ulSlotInput.value); }catch(_){} });
        ulWrap.appendChild(ulSlotInput);

        // Maksymalna liczba prób
        const ulTriesRow = document.createElement('div');
        ulTriesRow.style.display = 'flex';
        ulTriesRow.style.alignItems = 'center';
        ulTriesRow.style.gap = '8px';
        ulTriesRow.style.marginBottom = '6px';

        const ulTriesLabel = document.createElement('label');
        ulTriesLabel.htmlFor = 'adi-bot_upgrade_tries';
        ulTriesLabel.textContent = 'Maks. prób:';

        const ulTriesInput = document.createElement('input');
        ulTriesInput.type = 'number';
        ulTriesInput.min = '1';
        ulTriesInput.max = '999';
        ulTriesInput.id = 'adi-bot_upgrade_tries';
        ulTriesInput.className = 'adi-bot_inputs';
        ulTriesInput.style.width = '80px';
        ulTriesInput.placeholder = '10';
        try{ ulTriesInput.value = localStorage.getItem('adi-bot_upgrade_tries') || '10'; }catch(_){}
        ulTriesInput.addEventListener('change', ()=>{ try{ localStorage.setItem('adi-bot_upgrade_tries', ulTriesInput.value); }catch(_){} });

        ulTriesRow.appendChild(ulTriesLabel);
        ulTriesRow.appendChild(ulTriesInput);
        ulWrap.appendChild(ulTriesRow);

        // Licznik wykonanych prób
        const ulCountRow = document.createElement('div');
        ulCountRow.style.display = 'flex';
        ulCountRow.style.alignItems = 'center';
        ulCountRow.style.gap = '8px';
        ulCountRow.style.marginBottom = '8px';

        const ulCountLabel = document.createElement('span');
        ulCountLabel.textContent = 'Wykonane próby:';

        const ulCountVal = document.createElement('span');
        ulCountVal.id = 'adi-bot_upgrade_count_display';
        ulCountVal.style.fontWeight = '700';
        ulCountVal.textContent = localStorage.getItem('adi-bot_upgrade_count') || '0';

        const ulResetBtn = document.createElement('button');
        ulResetBtn.textContent = 'Resetuj licznik';
        ulResetBtn.className = 'adi-bot_inputs';
        ulResetBtn.style.width = 'auto';
        ulResetBtn.style.padding = '2px 10px';
        ulResetBtn.style.margin = '0';
        ulResetBtn.addEventListener('click', ()=>{
          try{
            localStorage.setItem('adi-bot_upgrade_count', '0');
            ulCountVal.textContent = '0';
            message('Ulepszanie: licznik zresetowany.');
          }catch(_){}
        });

        ulCountRow.appendChild(ulCountLabel);
        ulCountRow.appendChild(ulCountVal);
        ulCountRow.appendChild(ulResetBtn);
        ulWrap.appendChild(ulCountRow);

        // Status
        const ulStatus = document.createElement('div');
        ulStatus.id = 'adi-bot_upgrade_status';
        ulStatus.style.fontSize = '12px';
        ulStatus.style.marginBottom = '8px';
        ulStatus.style.color = '#555';
        ulStatus.textContent = 'Gotowe.';
        ulWrap.appendChild(ulStatus);

        // Przycisk: Ulepsz teraz
        const ulBtn = document.createElement('button');
        ulBtn.id = 'adi-bot_upgrade_btn';
        ulBtn.textContent = 'Ulepsz teraz (1 raz)';
        ulBtn.className = 'adi-bot_inputs';
        ulBtn.style.marginBottom = '4px';
        ulWrap.appendChild(ulBtn);

        // Logika ulepszania
        ulBtn.addEventListener('click', ()=>{
          try{
            const type = ulTypeSel.value;
            const itemName = ulSlotInput.value.trim();
            if(type === 'none' || !itemName){
              ulStatus.textContent = 'Ustaw typ ulepszenia i nazwę przedmiotu.';
              ulStatus.style.color = '#e57373';
              return;
            }

            // Znajdź item w ekwipunku (g.item)
            if(!window.g || !g.item){
              ulStatus.textContent = 'Brak danych o ekwipunku (g.item).';
              ulStatus.style.color = '#e57373';
              return;
            }

            const normName = s => String(s||'').toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/\s+/g,' ').trim();
            const needle = normName(itemName);
            let foundSlot = null;
            for(const slot in g.item){
              const it = g.item[slot];
              if(!it) continue;
              const nm = normName(it.name || it.nick || '');
              if(nm.includes(needle)){ foundSlot = slot; break; }
            }

            if(foundSlot === null){
              ulStatus.textContent = 'Nie znaleziono przedmiotu: ' + itemName;
              ulStatus.style.color = '#e57373';
              return;
            }

            // Wyślij żądanie ulepszenia
            _g('moveitem&id=' + foundSlot + '&st=1');

            const cur = parseInt(localStorage.getItem('adi-bot_upgrade_count')||'0', 10) + 1;
            localStorage.setItem('adi-bot_upgrade_count', String(cur));
            ulCountVal.textContent = String(cur);

            ulStatus.textContent = 'Użyto ulepszenia na ' + itemName + ' (łącznie: ' + cur + ')';
            ulStatus.style.color = '#3cb371';
            message('[Ulepszanie] ' + type + ' na ' + itemName + ' (próba ' + cur + ')');
          }catch(e){
            ulStatus.textContent = 'Błąd: ' + (e && e.message || e);
            ulStatus.style.color = '#e57373';
