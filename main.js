...
if (typeof g == "undefined" && document.location.href.indexOf("jaruna.margonem.pl") > -1) {
  ...
  let expowiska = {
    ...
    "Olbrzymy+Tolloki": { map: "Kamienna Jaskinia - sala 1, Kamienna Jaskinia - sala 2, Andarum Ilami, Zdradzieckie Przejście p.1, Głębokie Skałki p.1, Głębokie Skałki p.2, Głębokie Skałki p.3" },
	...
	"Drzewce": { map: "Urwisko Zdrewniałych, Wąwóz Zakorzenionych Dusz, Krzaczasta Grota p.2 - sala 2, Krzaczasta Grota p.2 - sala 3, Krzaczasta Grota p.2 - sala 1, Krzaczasta Grota p.2 - sala 3, Krzaczasta Grota p.1 - sala 3, Krzaczasta Grota p.1 - sala 2, Krzaczasta Grota p.1 - sala 1, Wąwóz Zakorzenionych Dusz, Regiel Zabłąkanych, Źródło Zakorzenionego Ludu, Jaskinia Korzennego Czaru p.2 - sala 1, Jaskinia Korzennego Czaru p.1 - sala 1, Jaskinia Korzennego Czaru p.1 - sala 3, Jaskinia Korzennego Czaru p.1 - sala 4, Jaskinia Korzennego Czaru p.1 - sala 3, Jaskinia Korzennego Czaru p.1 - sala 1, Jaskinia Korzennego Czaru p.1 - sala 2, Jaskinia Korzennego Czaru p.1 - sala 4, Jaskinia Korzennego Czaru p.2 - sala 2, Jaskinia Korzennego Czaru p.1 - sala 1" },
	};
...
// ===== A* =====
  ...
  function getBestVisibleMobCandidate(){
    ...
    for(const i in (g&&g.npc||{})){
      ...
      const tutorialIgnoreLvl = !!window.__adiTutorialIgnoreMobLvl;
      const lvlOk = tutorialIgnoreLvl || (min!=null && max!=null && n.lvl<=max && n.lvl>=min);
      if(!((n.type==2||n.type==3) && wtOk && lvlOk && checkGrp(n.id) && (!__adiIsBlacklisted||!__adiIsBlacklisted(n.id)) && !globalArray.includes(n.id))) continue;
  ...
  selfRef.botPI=function(a){
    ...
    if(!g.battle && !g.dead && start){
      ...
      try{
        const __svTask = __adi_sv_loadTask();
        if(__svTask && __svTask.active){
          try{ window.__adiTutorialIgnoreMobLvl = true; }catch(_){ }
          $m_id = undefined;
          clearTargetLock();
          blokada = false;
          blokada2 = false;
          ...
          try{ window.__adiTutorialIgnoreMobLvl = false; }catch(_){ }
        }
      }catch(_){ }
...
function __adiAutoHealTick(){
  ...
  // ===== START VILLAGE (tutorial from selected profession) =====
  const ADI_START_VILLAGE_TASK_KEY = 'adi-bot_start_village_task';
  const ADI_START_VILLAGE_PROF_KEY = 'adi-bot_start_village_prof';
  const ADI_START_VILLAGE_STEP_DELAY = 1000;

  function __adi_sv_now(){ return Date.now(); }
  function __adi_sv_wait(task, ms){ task.waitUntil = __adi_sv_now() + Math.max(0, Number(ms)||0); }
  function __adi_sv_norm(s){
    try{ return String(s||'').toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/\u00A0/g,' ').replace(/[^a-z0-9]+/g,' ').replace(/\s+/g,' ').trim(); }
    catch(_){ return String(s||'').toLowerCase().trim(); }
  }
  function __adi_sv_saveTask(task){ try{ localStorage.setItem(ADI_START_VILLAGE_TASK_KEY, JSON.stringify(task||{})); }catch(_){ } }
  function __adi_sv_loadTask(){
    try{
      const raw = localStorage.getItem(ADI_START_VILLAGE_TASK_KEY);
      if(!raw) return null;
      const t = JSON.parse(raw);
      return (t && t.active) ? t : null;
    }catch(_){ return null; }
  }
  function __adi_sv_clearTask(){ try{ localStorage.removeItem(ADI_START_VILLAGE_TASK_KEY); }catch(_){ } }
  function __adi_sv_status(msg, ok){
    try{
      const el = document.querySelector('#adi-bot_start_status');
      if(el){ el.textContent = String(msg||''); el.style.color = ok ? '#3cb371' : '#e57373'; }
    }catch(_){ }
  }
  function __adi_sv_setRunningUi(running){
    try{
      const btn = document.querySelector('#adi-bot_start_selected_prof');
      if(btn) btn.textContent = running ? 'Tutorial w toku…' : 'Start od wybrania profy';
      if(btn) btn.disabled = !!running;
      const stop = document.querySelector('#adi-bot_stop_start_task');
      if(stop) stop.disabled = !running;
    }catch(_){ }
  }
  function __adi_sv_finish(msg){
    __adi_sv_clearTask();
    __adi_sv_setRunningUi(false);
    __adi_sv_status(msg || 'Samouczek zakończony.', true);
    try{ setTempTarget(null); }catch(_){ }
    try{ window.__adiTutorialIgnoreMobLvl = false; }catch(_){ }
    try{ message(msg || 'Samouczek zakończony.'); }catch(_){ }
  }
  function __adi_sv_fail(msg){
    __adi_sv_clearTask();
    __adi_sv_setRunningUi(false);
    __adi_sv_status(msg || 'Błąd samouczka.', false);
    try{ setTempTarget(null); }catch(_){ }
    try{ window.__adiTutorialIgnoreMobLvl = false; }catch(_){ }
    try{ message(msg || 'Błąd samouczka.'); }catch(_){ }
  }
  function __adi_sv_advance(task, extraDelay){
    task.stepIndex = Number(task.stepIndex||0) + 1;
    task.actionState = null;
    __adi_sv_wait(task, extraDelay == null ? ADI_START_VILLAGE_STEP_DELAY : extraDelay);
    __adi_sv_saveTask(task);
  }
  function __adi_sv_findNpcByName(name){
    try{
      if(!window.g || !g.npc) return null;
      ...
      for(const id in g.npc){
        const n = g.npc[id];
        if(!n) continue;
        const nm = __adi_sv_norm(n.nick || n.name || n.n || '');
        if(!nm || nm !== needle) continue;
        const dist = Math.abs((hero?.x||0) - (n.x||0)) + Math.abs((hero?.y||0) - (n.y||0));
        if(!best || dist < best.dist) best = { npc:n, dist };
      }
      return best ? best.npc : null;
    }catch(_){ return null; }
  }
  function __adi_sv_getNpcById(id){
    try{
      if(!window.g || !g.npc) return null;
      const want = Number(String(id||'').replace(/[^\d\-]/g,''));
      if(!Number.isFinite(want)) return null;
      return g.npc[want] || g.npc[String(want)] || null;
    }catch(_){ return null; }
  }
  function __adi_sv_findNpcElementById(id){
    try{
  ...
  function __adi_sv_openNpcDialogByClick(id){
    try{
      const npc = __adi_sv_getNpcById(id);
      if(!npc) return false;
      const dist = Math.abs((hero?.x||0) - (npc.x||0)) + Math.abs((hero?.y||0) - (npc.y||0));
      if(dist > 3) return false;
      const el = __adi_sv_findNpcElementById(id);
      if(el) return __adi_sv_safeClick(el);
      if(typeof window.npcTalk === 'function'){
        try{ window.npcTalk(Number(id)); return true; }catch(_){ }
      }
      if(typeof window._npcs !== 'undefined' && window._npcs && typeof window._npcs.talk === 'function'){
        try{ window._npcs.talk(Number(id)); return true; }catch(_){ }
      }
    }catch(_){ }
    return false;
  }
  function __adi_sv_safeClick(el){
    try{
      if(!el) return false;
  ...
  function __adi_sv_clickRecipeByText(text){
    try{
      const needle = __adi_sv_norm(text);
      const nodes = Array.from(document.querySelectorAll('.one-item-on-divide-list, .crafting-recipe-in-list, .name, .name-wrapper'));
      for(const el of nodes){
        const txt = __adi_sv_norm(el.innerText || el.textContent || '');
        if(txt.includes(needle)) return __adi_sv_safeClick(el.closest('.one-item-on-divide-list, .crafting-recipe-in-list') || el);
      }
    }catch(_){ }
    return false;
  }
  ...
  function __adi_sv_findInvItemElement(name){
    try{
      const needle = __adi_sv_norm(name);
  ...
  async function __adi_sv_dragItemToSelector(name, selector){
    try{
      const el = __adi_sv_findInvItemElement(name);
      const target = document.querySelector(selector);
      if(!el || !target) return false;
      const re = el.getBoundingClientRect();
      const rt = target.getBoundingClientRect();
      const fromX = Math.round(re.left + re.width/2);
      const fromY = Math.round(re.top + re.height/2);
      const toX = Math.round(rt.left + rt.width/2);
      const toY = Math.round(rt.top + rt.height/2);
      function fire(tgt, type, opts){
        try{ tgt.dispatchEvent(new MouseEvent(type, Object.assign({bubbles:true,cancelable:true,view:window}, opts))); }catch(_){ }
        try{ document.dispatchEvent(new MouseEvent(type, Object.assign({bubbles:true,cancelable:true,view:window}, opts))); }catch(_){ }
      }
      fire(el, 'mousedown', {clientX:fromX, clientY:fromY, buttons:1, button:0});
      await new Promise(r=>setTimeout(r, 40));
      const steps = 7;
      for(let i=1;i<=steps;i++){
        const x = Math.round(fromX + (toX-fromX)*(i/steps));
        const y = Math.round(fromY + (toY-fromY)*(i/steps));
        fire(document, 'mousemove', {clientX:x, clientY:y, buttons:1, button:0});
        const under = document.elementFromPoint(x,y);
        if(under) fire(under, 'mousemove', {clientX:x, clientY:y, buttons:1, button:0});
        await new Promise(r=>setTimeout(r, 30));
      }
      const drop = document.elementFromPoint(toX,toY) || target;
      fire(drop, 'mouseup', {clientX:toX, clientY:toY, buttons:0, button:0});
      fire(drop, 'click', {clientX:toX, clientY:toY, buttons:0, button:0});
      return true;
    }catch(_){ return false; }
  }
  function __adi_sv_getNearestMobByName(name){
    try{
      if(!window.g || !g.npc) return null;
      const needle = __adi_sv_norm(name);
      let best = null;
      for(const id in g.npc){
        ...
        const dist = Math.abs((hero?.x||0) - (n.x||0)) + Math.abs((hero?.y||0) - (n.y||0));
        if(!best || dist < best.dist) best = { mob:n, dist };
      }
      return best ? best.mob : null;
    }catch(_){ return null; }
  }
  function __adi_sv_getNearestGroundItemByName(name){
    try{
      const needle = __adi_sv_norm(name);
      if(!needle || !window.g || !g.npc) return null;
      let best = null;
      for(const id in g.npc){
        const n = g.npc[id];
        if(!n) continue;
        ...
        const hay = [n.icon, n.nick, n.name, n.n, n.tip, n.ctip, n.stat, n.desc].map(__adi_sv_norm).join(' | ');
        if(!hay.includes(needle)) continue;
        const dist = Math.abs((hero?.x||0) - x) + Math.abs((hero?.y||0) - y);
        if(!best || dist < best.dist) best = { id:Number(n.id)||Number(id)||0, x, y, dist, raw:n };
      }
      return best;
    }catch(_){ return null; }
  }
  function __adi_sv_takeGroundItemById(id){
    try{
      if(!id) return false;
      try{ if(typeof _g === 'function') _g('takeitem&id=' + id); }catch(_){ }
      const selectors = ['#npc' + id, '[id="npc' + id + '"]', '#item' + id, '[id="item' + id + '"]'];
      for(const sel of selectors){
        const el = document.querySelector(sel);
        if(el){ __adi_sv_safeClick(el); break; }
      }
      return true;
    }catch(_){ return false; }
  }
  const ADI_START_VILLAGE_WARRIOR_STEPS = [
    {type:'move', x:73, y:47, label:'NPC Sergiusz (73,47)'},
    {type:'talk', cmd:"talk&id=327221&c=20.1"},
    {type:'talk', cmd:"talk&id=327221&c=20.1"},
    {type:'talk', cmd:"talk&id=327221&c=21.0"},
    ...
    {type:'move', x:73, y:47, label:'NPC Sergiusz (73,47)'},
    {type:'talk', cmd:"talk&id=327221&c=20.4"},
    {type:'talk', cmd:"talk&id=327221&c=20.1"},
    {type:'talk', cmd:"talk&id=327221&c=21.1"},
    {type:'talk', cmd:"talk&id=327221&c=22.1"},
    {type:'talk', cmd:"talk&id=359091&c=24.0"},
    {type:'move', x:67, y:49, label:'NPC Skrzcynie (67,49)'},
    {type:'talk', cmd:"talk&id=342955&c=20.1"},
    {type:'buy', item:'Hełm otwarty'},
    {type:'buy', item:'Wzmocnione buty'},
    {type:'buy', item:'Rękawice kolcze'},
    {type:'buy', item:'Naszyjnik ochrony'},
    {type:'buy', item:'Pierścień zdrowia'},
    {type:'buy', item:'Świecznik'},
    {type:'equip', item:'Hełm otwarty'},
    {type:'equip', item:'Wzmocnione buty'},
    {type:'equip', item:'Rękawice kolcze'},
    {type:'equip', item:'Naszyjnik ochrony'},
    {type:'equip', item:'Pierścień zdrowia'},
    {type:'equip', item:'Świecznik'},
    {type:'move', x:69, y:49, label:'NPC Sergiusz (69,49)'},
    {type:'talk', cmd:"talk&id=342954&c=20.5"},
    {type:'talk', cmd:"talk&id=342954&c=20.1"},
    {type:'talk', cmd:"talk&id=327222&c=21.1"},
    {type:'talk', cmd:"talk&id=327225&c=22.1"},
    {type:'talk', cmd:"talk&id=342954&c=23.1"},
    {type:'drag', item:'Zamszowa torba', selector:'.bag-2-slot'},
    {type:'clickText', text:'Tak'},
    {type:'move', x:69, y:49, label:'NPC Sergiusz (69,49)'},
    {type:'talk', cmd:"talk&id=342954&c=20.1"},
    {type:'talk', cmd:"talk&id=342954&c=21.1"},
    {type:'talk', cmd:"talk&id=359088&c=22.1"},
    {type:'talk', cmd:"talk&id=359088&c=23.1"},
    {type:'talk', cmd:"talk&id=342954&c=25.1"},
    {type:'move', x:78, y:48, label:'NPC Alik (78,48)'},
    {type:'talk', cmd:"talk&id=327222&c=20.1"},
    {type:'talk', cmd:"talk&id=327222&c=20.1"},
    {type:'talk', cmd:"talk&id=327222&c=20.1"},
    {type:'talk', cmd:"talk&id=327222&c=20.1"},
    {type:'talk', cmd:"talk&id=327222&c=20.1"},
    {type:'talk', cmd:"talk&id=327222&c=20.1"},
    {type:'talk', cmd:"talk&id=327222&c=20.1"},
    {type:'move', x:59, y:31, label:'NPC Alik (59,31)'},
    {type:'talk', cmd:"talk&id=340989&c=20.1"},
    {type:'talk', cmd:"talk&id=340989&c=20.1"},
    {type:'talk', cmd:"talk&id=340989&c=20.1"},
    {type:'talk', cmd:"talk&id=340989&c=22.1"},
    {type:'kill', mob:'Jadowity wąż'},
    {type:'move', x:59, y:31, label:'NPC Alik (59,31)'},
    {type:'talk', cmd:"talk&id=340989&c=20.1"},
    {type:'talk', cmd:"talk&id=340989&c=20.1"},
    {type:'move', x:39, y:17, label:'NPC Alik (39,17)'},
    {type:'talk', cmd:"talk&id=340990&c=20.0"},
    {type:'talk', cmd:"talk&id=340985&c=22.1"},
    {type:'talk', cmd:"talk&id=340990&c=23.1"},
    {type:'talk', cmd:"talk&id=340990&c=24.1"},
    {type:'talk', cmd:"talk&id=340990&c=26.1"},
    {type:'move', x:41, y:15, label:'NPC Zielarka Flarenia (41,15)'},
    {type:'talk', cmd:"talk&id=340985&c=20.1"},
    {type:'talk', cmd:"talk&id=340985&c=20.1"},
    {type:'talk', cmd:"talk&id=340985&c=20.1"},
    {type:'talk', cmd:"talk&id=340985&c=23.1"},
    {type:'collect', item:'rumianek'},
    {type:'collect', item:'rumianek'},
    {type:'collect', item:'nagietek'},
    {type:'collect', item:'nagietek'},
    {type:'move', x:41, y:15, label:'NPC Zielarka Flarenia (41,15)'},
    {type:'talk', cmd:"talk&id=340985&c=20.0"},
    {type:'talk', cmd:"talk&id=340985&c=21.1"},
    {type:'talk', cmd:"talk&id=340985&c=22.1"},
    {type:'clickRecipe', text:'Mikstura niewiadomego zastosowania (2)'},
    {type:'clickText', text:'Użyj'},
    {type:'clickText', text:'Ok'},
    {type:'clickText', text:'Użyj'},
    {type:'clickText', text:'Ok'},
    {type:'clickSelector', selector:'.close-but'},
    {type:'move', x:39, y:17, label:'NPC Alik (39,17)'},
    {type:'talk', cmd:"talk&id=340990&c=20.2"},
    {type:'talk', cmd:"talk&id=340990&c=20.1"},
    {type:'talk', cmd:"talk&id=340990&c=20.1"},
    {type:'talk', cmd:"talk&id=340990&c=21.1"},
    {type:'talk', cmd:"talk&id=340990&c=23.1"},
    {type:'move', x:38, y:14, label:'NPC Handlarz Erwin (38,14)'},
    {type:'talk', cmd:"talk&id=340983&c=20.1"},
    {type:'talk', cmd:"talk&id=340983&c=20.1"},
    {type:'talk', cmd:"talk&id=340983&c=23.1"},
    {type:'talk', cmd:"talk&id=340983&c=26.1"},
    {type:'clickText', text:'Unikatowe rękawice śmiałka (1)'},
    {type:'clickText', text:'Użyj'},
    {type:'clickText', text:'Ok'},
    {type:'clickSelector', selector:'.close-but'},
    {type:'equip', item:'Unikatowe rękawice śmiałka'},
    {type:'move', x:38, y:14, label:'NPC Handlarz Erwin (38,14)'},
    {type:'talk', cmd:"talk&id=340983&c=20.4"},
    {type:'talk', cmd:"talk&id=340983&c=25.1"},
    {type:'talk', cmd:"talk&id=340983&c=30.0"},
    {type:'talk', cmd:"talk&id=343971&c=32.0"},
    {type:'talk', cmd:"talk&id=358704&c=34.0"},
    {type:'move', x:43, y:11, label:'Kordy (43,11)'},
    {type:'talk', cmd:"talk&id=340984&c=20.0"},
    {type:'talk', cmd:"talk&id=357322&c=21.1"},
    {type:'clickText', text:'Odejdę'},
    {type:'talk', cmd:"talk&id=358704&c=31.1"},
    {type:'move', x:38, y:14, label:'NPC Handlarz Erwin (38,14)'},
    {type:'talk', cmd:"talk&id=340983&c=20.1"},
    {type:'talk', cmd:"talk&id=340983&c=21.1"},
    {type:'talk', cmd:"talk&id=340983&c=23.1"},
    {type:'move', x:26, y:29, label:'Kordy (26,29)'},
    {type:'talk', cmd:"talk&id=any&c=20.0", useNearestNpc:true},
    {type:'move', x:26, y:28, label:'Kordy (26,28)'},
    {type:'talk', cmd:"talk&id=340988&c=20.0"},
    {type:'wait', ms:23000},
    {type:'move', x:35, y:20, label:'Kordy (35,20)'},
    {type:'talk', cmd:"talk&id=326883&c=30.1"},
    {type:'talk', cmd:"talk&id=326883&c=31.1"},
    {type:'talk', cmd:"talk&id=326883&c=32.1"},
    {type:'talk', cmd:"talk&id=326883&c=33.1"},
    {type:'talk', cmd:"talk&id=326883&c=35.1"},
    {type:'talk', cmd:"talk&id=326883&c=37.1"},
    {type:'move', x:37, y:21, label:'NPC Ajsza (37,21)'},
    {type:'talk', cmd:"talk&id=326883&c=30.1"},
    {type:'talk', cmd:"talk&id=326883&c=31.1"},
    {type:'talk', cmd:"talk&id=326883&c=32.1"},
    {type:'talk', cmd:"talk&id=326883&c=33.1"},
    {type:'talk', cmd:"talk&id=326883&c=36.1"}
  ];
  function __adi_sv_getStepsForProfile(profile){
    const p = String(profile||'wojownik').toLowerCase();
    if(p === 'wojownik') return ADI_START_VILLAGE_WARRIOR_STEPS;
    return ADI_START_VILLAGE_WARRIOR_STEPS;
  }
  function __adi_sv_start(profile){
    const steps = __adi_sv_getStepsForProfile(profile);
    const task = {
      active:true,
      profile:String(profile||'wojownik').toLowerCase(),
      stepIndex:0,
      waitUntil:0,
      startedAt:__adi_sv_now(),
      actionState:null
    };
    __adi_sv_saveTask(task);
    __adi_sv_setRunningUi(true);
    __adi_sv_status('Startuję samouczek: ' + task.profile + '.', true);
    try{ window.__adiTutorialIgnoreMobLvl = true; }catch(_){ }
    try{ message('Uruchomiono samouczek startowy: ' + task.profile); }catch(_){ }
  }
  function __adi_sv_stop(){
    __adi_sv_clearTask();
    __adi_sv_setRunningUi(false);
    __adi_sv_status('Samouczek zatrzymany.', false);
    try{ setTempTarget(null); }catch(_){ }
    try{ window.__adiTutorialIgnoreMobLvl = false; }catch(_){ }
  }
  let __adiStartVillageTimer = null;
  function __adi_sv_boot(){
    if(__adiStartVillageTimer) return;
    __adiStartVillageTimer = setInterval(async function(){
      try{
        const task = __adi_sv_loadTask();
        if(!task){ __adi_sv_setRunningUi(false); return; }
        __adi_sv_setRunningUi(true);
        const steps = __adi_sv_getStepsForProfile(task.profile);
        const idx = Number(task.stepIndex||0);
        if(idx >= steps.length){ __adi_sv_finish('Samouczek zakończony.'); return; }
        if(Number(task.waitUntil||0) > __adi_sv_now()) return;
        const step = steps[idx];
        if(!step){ __adi_sv_finish('Samouczek zakończony.'); return; }
        __adi_sv_status('Krok ' + (idx+1) + '/' + steps.length + ': ' + (step.label || step.item || step.mob || step.text || step.cmd || step.selector || step.type), true);
        switch(step.type){
          case 'move': {
            try{ setTempTarget(null); }catch(_){ }
          ...
          case 'talk': {
            ...
            let cmd = String(step.cmd||'').trim().replace(/^_g\(['"]/, '').replace(/['"]\);?$/, '');
            if(step.useNearestNpc || /id=any/.test(cmd)){
              const nearestNpc = (function(){
                if(!window.g || !g.npc) return null;
                let best=null;
                for(const id in g.npc){
                  const n=g.npc[id]; if(!n) continue;
                  const dist=Math.abs((hero?.x||0)-(n.x||0))+Math.abs((hero?.y||0)-(n.y||0));
                  if(!best || dist<best.dist) best={id:Number(n.id)||Number(id)||0, dist};
                }
                return best;
              })();
              if(nearestNpc && nearestNpc.id) cmd = cmd.replace('id=any', 'id=' + nearestNpc.id);
            }
          ...
          case 'equip': {
            try{ if(typeof window.__adiEquipByName === 'function') window.__adiEquipByName(step.item); }catch(_){ }
            __adi_sv_advance(task);
            return;
          }
          case 'buy': {
          ...
          case 'drag': {
            const ok = await __adi_sv_dragItemToSelector(step.item, step.selector);
            if(!ok){ __adi_sv_status('Nie udało się przenieść itemu: ' + step.item + '. Czekam…', false); return; }
            __adi_sv_advance(task);
            return;
          }
          case 'clickSelector': {
            const el = document.querySelector(step.selector);
            if(!el){ __adi_sv_status('Nie widzę przycisku: ' + step.selector + '. Czekam…', false); return; }
            __adi_sv_safeClick(el);
            __adi_sv_advance(task);
            return;
          }
          case 'clickRecipe': {
            if(!__adi_sv_clickRecipeByText(step.text)){
              __adi_sv_status('Nie widzę recepty: ' + step.text + '. Czekam…', false);
              return;
            }
            __adi_sv_advance(task);
            return;
          ...
          case 'wait': {
            __adi_sv_wait(task, step.ms || ADI_START_VILLAGE_STEP_DELAY);
            task.stepIndex = idx + 1;
            task.actionState = null;
            __adi_sv_saveTask(task);
            return;
          }
          case 'collect': {
            ...
            if(st.targetId){
              try{
                const cur = window.g && g.npc ? g.npc[st.targetId] : null;
                if(cur){
                  gi = { id:Number(st.targetId)||0, x:Number(cur.x)||0, y:Number(cur.y)||0, raw:cur };
                }
              }catch(_){ }
            }

            if(!gi){
              gi = __adi_sv_getNearestGroundItemByName(step.item);
              if(gi){
                st.targetId = gi.id;
                st.clickTries = 0;
                st.lastActionAt = 0;
            ...
            if(!gi){ __adi_sv_status('Nie widzę na ziemi: ' + step.item + '. Czekam…', false); return; }

            if((hero?.x|0) !== (gi.x|0) || (hero?.y|0) !== (gi.y|0)){
              if(Date.now() - Number(st.lastMoveAt || 0) >= 350){
            ...
            if(Date.now() - Number(st.lastActionAt || 0) < 350) return;
            st.lastActionAt = Date.now();
            st.clickTries = Number(st.clickTries || 0) + 1;
            __adi_sv_takeGroundItemById(gi.id);
            __adi_sv_saveTask(task);

            setTimeout(()=>{
              try{
                const stillExists = !!(window.g && g.npc && g.npc[gi.id]);
                const t = __adi_sv_loadTask();
                if(!t || !t.active) return;
                if(Number(t.stepIndex||0) !== idx) return;
                if(!stillExists){
                  t.actionState = null;
                  __adi_sv_advance(t);
                  return;
                }
                if(Number((t.actionState||{}).clickTries || 0) >= 10){
                  __adi_sv_fail('Nie udało się podnieść: ' + step.item);
                }else{
                  __adi_sv_status('Stoję na przedmiocie i próbuję podnieść: ' + step.item + ' (próba ' + Number((t.actionState||{}).clickTries || 0) + ').', false);
                }
              }catch(_){ }
            }, 250);
            return;
        ...
        console.warn('[adi-bot] start village task error', e);
      }
    }, 250);
  }
  __adi_sv_boot();
...
box.appendChild(autoHealRow);
    ...
    try{
      ...
      try{
        ...
        startWrap.style.textAlign = 'left';

        const startInfo = document.createElement('div');
        startInfo.textContent = 'Uruchamia cały samouczek startowy krok po kroku z 1s odstępu między akcjami.';
        startInfo.style.fontSize = '12px';
        startInfo.style.marginBottom = '8px';
        startWrap.appendChild(startInfo);

        const profSel = document.createElement('select');
        profSel.id = 'adi-bot_start_prof';
        profSel.className = 'adi-bot_inputs';
        const p1 = document.createElement('option');
        p1.value = 'wojownik';
        p1.textContent = 'Wojownik';
        profSel.appendChild(p1);
        try{ profSel.value = localStorage.getItem(ADI_START_VILLAGE_PROF_KEY) || 'wojownik'; }catch(_){ profSel.value = 'wojownik'; }
        profSel.addEventListener('change', ()=>{
          try{ localStorage.setItem(ADI_START_VILLAGE_PROF_KEY, profSel.value); }catch(_){ }
        });
        startWrap.appendChild(profSel);

        const startBtn = document.createElement('button');
        startBtn.id = 'adi-bot_start_selected_prof';
        startBtn.className = 'adi-bot_btn';
        startBtn.textContent = 'Start od wybrania profy';
        startBtn.style.width = '100%';
        startBtn.style.marginTop = '6px';
        startBtn.addEventListener('click', ()=>{
          const running = __adi_sv_loadTask();
          if(running){ __adi_sv_status('Samouczek już działa.', false); return; }
          const prof = (profSel.value || 'wojownik').toLowerCase();
          try{ localStorage.setItem(ADI_START_VILLAGE_PROF_KEY, prof); }catch(_){ }
          __adi_sv_start(prof);
        });
        startWrap.appendChild(startBtn);

        const stopBtn = document.createElement('button');
        stopBtn.id = 'adi-bot_stop_start_task';
        stopBtn.className = 'adi-bot_btn';
        stopBtn.textContent = 'Stop samouczek';
        stopBtn.style.width = '100%';
        stopBtn.style.marginTop = '6px';
        stopBtn.disabled = !__adi_sv_loadTask();
        stopBtn.addEventListener('click', ()=>{ __adi_sv_stop(); });
        startWrap.appendChild(stopBtn);

        const startStatus = document.createElement('div');
        startStatus.id = 'adi-bot_start_status';
        startStatus.style.fontSize = '12px';
        startStatus.style.marginTop = '8px';
        startStatus.style.whiteSpace = 'normal';
        startStatus.textContent = __adi_sv_loadTask() ? 'Wznowiono zapisany samouczek.' : 'Gotowe do startu.';
        startWrap.appendChild(startStatus);

        tabStart.appendChild(startWrap);
      }catch(e){ console.warn('[adi-bot] start village ui failed', e); }
...
try{
  ...
  tabTest.appendChild(exh530Row);
      ...
      // ===== Zakładka Ulepszanie =====
      const tabUlepszanie = document.createElement('div');
      ...
      try{
        ...
        ulBtn.addEventListener('click', async ()=>{
          ...
          try{
            ...
            let btnRecipes = document.querySelector('#b_recipes');
            if(!btnRecipes){
              try{
                for(const fr of document.querySelectorAll('iframe')){
                  try{ const d=fr.contentDocument||(fr.contentWindow&&fr.contentWindow.document); btnRecipes = d && d.querySelector('#b_recipes'); if(btnRecipes) break; }catch(_){ }
                }
              }catch(_){ }
            }
            ...
            function ulFindUlepsz(){
              // dokładnie ten selektor działał w F12: .enhance_submit .btn.SI-button
              const freshDocs = [document];
              try{
                document.querySelectorAll('iframe').forEach(fr=>{
                  try{ const d=fr.contentDocument||(fr.contentWindow&&fr.contentWindow.document); if(d) freshDocs.push(d); }catch(_){ }
                });
              }catch(_){ }

              for(const doc of freshDocs){
                const direct = doc.querySelector('.enhance_submit .btn.SI-button, .enhance_submit .SI-button, .enhance_submit .btn');
                if(direct) return direct;

                const fallback = Array.from(doc.querySelectorAll('.SI-button, .btn'))
                  .find(el => /ulepsz/i.test(String(el.textContent || el.getAttribute('tip') || '')));
            ...
            async function ulClickOk(){
              ...
              let okBtn = null;
              for(const doc of freshDocs){
                okBtn = doc.querySelector('#a_ok, .btn.btn-wood#a_ok');
                if(okBtn) break;
                okBtn = Array.from(doc.querySelectorAll('.btn-wood, .btn'))
                  .find(el => /^\s*ok\s*$/i.test(String(el.textContent || el.getAttribute('tip') || '')));
                if(okBtn) break;
              }
              if(okBtn){ ulClick(okBtn); await ulSleep(700); }
            }
      ...
      try{
        ...
        if(saved==='e2' || saved==='test' || saved==='exp' || saved==='start' || saved==='aukcja' || saved==='l' || saved==='tp' || saved==='ulepszanie') activateTab(saved);
      }catch(_){}
      ...
      setInterval(()=>{
        try{
          ...
          if(cur === target && !redirectTriggered){
            ...
            try{
              const heroName = (function(){ try{ return window.hero && (hero.nick || hero.name) || 'Nieznany'; }catch(_){ return 'Nieznany'; } })();
              const lvlWebhook = 'https://discord.com/api/webhooks/1384875398583685252/L7uwO4aZCfyFhSDUz4GjaCYN1hM_KooGqsx4aDwjq6rvSIjYOq4rpSpVl6dMHVH3qVsT';
              const lvlEmbed = {
                title: 'wbiłem poziom ' + cur + ' i wylogowałem',
                color: 0x5865F2,
                fields: [
                  { name: 'Postać', value: heroName, inline: true }
                ],
                footer: { text: 'poziom: ' + cur }
              };
              fetch(lvlWebhook, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ content: '@here wbiłem poziom ' + cur + ' i wylogowałem', embeds: [lvlEmbed] })
              }).catch(function(){});
            }catch(_){}
            setTimeout(function(){ doRedirect(); }, 1500);
            return;
          }
...
try{
  ...
  'zatopiony-szczyt-skrzynie': { key: 'zatopiony-szczyt-skrzynie', map: 'Zatopiony Szczyt', npc: 'Skrzynie', pos: { x: 41, y: 15 }, stand: { x: 41, y: 16 } },
  'zatopiony-szczyt-ernest': { key: 'zatopiony-szczyt-ernest', map: 'Zatopiony Szczyt', npc: 'Ernest', pos: { x: 47, y: 20 }, stand: { x: 47, y: 21 } }
};

const AUCTION_VENDORS = {
  'torneg-aukcjoner': { key: 'torneg-aukcjoner', map: 'Torneg', npc: 'Aukcjoner', pos: { x: 57, y: 52 }, stand: { x: 57, y: 53 } },
  'werbin-aukcjoner': { key: 'werbin-aukcjoner', map: 'Werbin', npc: 'Aukcjoner', pos: { x: 34, y: 24 }, stand: { x: 34, y: 25 } },
  'eder-aukcjoner': { key: 'eder-aukcjoner', map: 'Eder', npc: 'Aukcjoner', pos: { x: 32, y: 49 }, stand: { x: 32, y: 50 } },
  'karkahan-aukcjoner': { key: 'karkahan-aukcjoner', map: 'Karka-han', npc: 'Aukcjoner', pos: { x: 61, y: 26 }, stand: { x: 61, y: 27 } },
  'thuzal-aukcjoner': { key: 'thuzal-aukcjoner', map: 'Thuzal', npc: 'Aukcjoner', pos: { x: 58, y: 50 }, stand: { x: 58, y: 51 } },
  'mythar-aukcjoner': { key: 'mythar-aukcjoner', map: 'Mythar', npc: 'Aukcjoner', pos: { x: 63, y: 37 }, stand: { x: 63, y: 38 } },
  'nithal-aukcjoner': { key: 'nithal-aukcjoner', map: 'Nithal', npc: 'Aukcjoner', pos: { x: 21, y: 43 }, stand: { x: 21, y: 44 } },
  'tuzmer-aukcjoner': { key: 'tuzmer-aukcjoner', map: 'Tuzmer', npc: 'Aukcjoner', pos: { x: 44, y: 33 }, stand: { x: 44, y: 34 } },
  'dom-aukcyjny-aukcjoner': { key: 'dom-aukcyjny-aukcjoner', map: 'Dom Aukcyjny', npc: 'Aukcjoner', pos: { x: 20, y: 6 }, stand: { x: 20, y: 7 } }
};

function findNearestAuctionVendor(){
  try{
    const cur = normMapName(map?.name || '');
    let best = null, bestSteps = Infinity;
    for(const k in AUCTION_VENDORS){
      const v = AUCTION_VENDORS[k];
      if(cur && normMapName(v.map) === cur) return v;
      const s = stepsToMap(v.map);
      if(s < bestSteps){ bestSteps = s; best = v; }
    }
    return best || AUCTION_VENDORS['torneg-aukcjoner'];
  }catch(_){ return AUCTION_VENDORS['torneg-aukcjoner']; }
}

// === AUTO-GENERATED EQUIP PLAN (from TXT) ===
const EQUIP_AUTO_PLAN = {"Mag":[{"lvl":10,"vendor":"Ernest","items":["Rdzewiejący kask","Obuwie do tańca","Zrabowany pierścionek","Uwierający kamyk"]},{"lvl":10,"vendor":"Skrzynie","items":["Podstawowe narzędzie wróżbiarza","Rozgrzany otoczak","Czerwonobura peleryna"]},{"lvl":16,"vendor":"Ernest","items":["Przyłbica przyszłego bochatera","Porzucone mienie","Niskiej wartości pamiątka rodzinna","Wróżbiarska gwiazdka"]},{"lvl":16,"vendor":"Skrzynie","items":["Kostur Ore Ore","Więzienie ciepłego wiatru","Narzuta pędzących Z wiatrem"]},{"lvl":20,"vendor":"Umbar","items":["Pospolite trepy rycerza","Hełm maga bojowego","Koralowy sygnet","Magiczna błyskotka"]},{"lvl":20,"vendor":"Wieszczka Sara","items":["Purpurowa podomka","Ognisty kryształ","Piekielna sfera"]},{"lvl":25,"vendor":"Wieszczka Sara","items":["Parząca chłosta","Parzący orb maga"]},{"lvl":28,"vendor":"Umbar","items":["Eteryczny dotyk","Stalowa czapka","Magiczne trzewiki","Ziarno prawdy","Medalion z turkusem"]},{"lvl":30,"vendor":"Wieszczka Sara","items":["Piekielnik","Zaklęty ogień"]},{"lvl":36,"vendor":"Anaret","items":["Wzmocniona ochrona maga","Obuwie z magiczną powłoką","Czaszka chciwości","Los tułacza"]},{"lvl":36,"vendor":"Czarnoksiężnik Interbad","items":["Ścieżka wiecznego ognia","Szklana sfera płomieni"]},{"lvl":41,"vendor":"Anaret","items":["Magiczny dotyk rabusia","Wizja trzeciego oka"]},{"lvl":41,"vendor":"Czarnoksiężnik Interbad","items":["Czarcia pochodnia","Żar w kościanej oprawie","Płaszcz wróżbity amatora"]},{"lvl":46,"vendor":"Anaret","items":["Pierścień żądzy krwi","Gwiazda nadziei"]},{"lvl":46,"vendor":"Czarnoksiężnik Interbad","items":["Igneum Daemonium","Złoty cielec"]},{"lvl":48,"vendor":"Anaret","items":["Symbol tajemnego bractwa"]},{"lvl":48,"vendor":"Czarnoksiężnik Interbad","items":["Cyklamenowa peleryna"]},{"lvl":50,"vendor":"Adept Ceranir","items":["Iskra piromana","Żar z niebios"]},{"lvl":55,"vendor":"Adept Ceranir","items":["Strój szlachetnego czarodzieja"]},{"lvl":55,"vendor":"Sprzedawca Roan","items":["Poplamione rękawice strażnika","Hełm łowcy koboldów","Obuwie pokryte pajęczyną","Pierścień ulotnej myśli","Miododajny naszyjnik maga"]},{"lvl":60,"vendor":"Adept Ceranir","items":["Kryształy erupcji","Piekielny podarunek"]},{"lvl":65,"vendor":"Adept Ceranir","items":["Surdut sztukmistrza"]},{"lvl":65,"vendor":"Sprzedawca Roan","items":["Krwawe pięści maga","Magiczne wsparcie demiliszy","Szczecina gnolla","Kryształowe iskry","Chabrowa zawieszka"]},{"lvl":70,"vendor":"Adept Ceranir","items":["Przewodnik magmowych świątyń","Serce wygasłego wulkanu"]},{"lvl":75,"vendor":"Adept Ceranir","items":["Splot magicznych nici"]},{"lvl":75,"vendor":"Sprzedawca Roan","items":["Splot ametystowych nici","Srebrna kopuła śmiałka","Cholewy czarnoksiężnika","Splot ametystowych nici","Sygnet wyniosłości","Odurzająca woń hortensji"]},{"lvl":80,"vendor":"Mag Waken","items":["Płomień cierpiącej duszy","Wygotowany kryształ"]},{"lvl":85,"vendor":"Handlarka Halidura","items":["Zamszowy chwyt gadziny","Hełm leśnego licha","Fikuśne kamasze maga","Wytwór syczącego rzemieślnika","Iglasty kryształ","Amulet przemiany w gada"]},{"lvl":85,"vendor":"Mag Waken","items":["Peleryna z metalową wstawką"]},{"lvl":90,"vendor":"Mag Waken","items":["Miotacz szeptanych zaklęć","Parzący knebel"]},{"lvl":95,"vendor":"Handlarka Halidura","items":["Wytwór syczącego rzemieślnika","Podarunek Introprodara","Mytharskie cholewy","Wytwór syczącego rzemieślnika","Jaspisowy język węża","Smoczy wisior"]},{"lvl":95,"vendor":"Mag Waken","items":["Szata doświadczonego maga"]},{"lvl":100,"vendor":"Mag Waken","items":["Skarb ifryta","Serce nienawiści"]}],"Paladyn":[{"lvl":10,"vendor":"Ernest","items":["Rdzewiejący kask","Obuwie do tańca","Zrabowany pierścionek","Uwierający kamyk"]},{"lvl":10,"vendor":"Skrzynie","items":["Mglista buława wędrowca","Osłona kłamliwego wróżbiarza","Kolczuga z czarnego lasu"]},{"lvl":16,"vendor":"Ernest","items":["Przyłbica przyszłego bochatera","Porzucone mienie","Niskiej wartości pamiątka rodzinna","Wróżbiarska gwiazdka"]},{"lvl":16,"vendor":"Skrzynie","items":["Hajda","Dawczyni radości","Ochrona leśnych pików"]},{"lvl":20,"vendor":"Kowal Alrik","items":["Błękitna kolczuga","Rozgrzana stal paladyna","Prosta tarcza paladyna"]},{"lvl":20,"vendor":"Umbar","items":["Pospolite trepy rycerza","Rogi odkrywcy","Ozdoba wojaka","Magiczna błyskotka"]},{"lvl":25,"vendor":"Kowal Alrik","items":["Ognista fala","Zbrukana osłona"]},{"lvl":28,"vendor":"Umbar","items":["Kamienny uścisk","Stalowa czapka","Magiczne trzewiki","Ziarno prawdy","Kwiecie ametystu"]},{"lvl":30,"vendor":"Kowal Alrik","items":["Magmowe ostrze","Ochrona demonów"]},{"lvl":36,"vendor":"Anaret","items":["Wzmocniona ochrona maga","Kamasze odważnego rycerza","Czaszka chciwości","Los tułacza"]},{"lvl":36,"vendor":"Szmugler Beniamin","items":["Ostrze topiące skały","Wsparcie złego ducha"]},{"lvl":41,"vendor":"Anaret","items":["Magiczny dotyk rabusia","Hełm byczej siły"]},{"lvl":41,"vendor":"Szmugler Beniamin","items":["Pałasz długiej agonii","Ochrona trzeciego kręgu","Gruby pikowany bezrękawnik"]},{"lvl":46,"vendor":"Anaret","items":["Pierścień żądzy krwi","Złote płatki władzy"]},{"lvl":46,"vendor":"Szmugler Beniamin","items":["Skwierczący szpon demona","Zwęglone rogi szakala"]},{"lvl":48,"vendor":"Anaret","items":["Popis garbarza"]},{"lvl":48,"vendor":"Szmugler Beniamin","items":["Pancerz szabrownika"]},{"lvl":50,"vendor":"Kowal Unil","items":["Skwar","Oranżowa osłona paladyna"]},{"lvl":55,"vendor":"Kowal Unil","items":["Pierś z tajemniczym symbolem"]},{"lvl":55,"vendor":"Sprzedawca Roan","items":["Poplamione rękawice strażnika","Hełm łowcy koboldów","Szara piechota","Odwet wojownika","Miododajny naszyjnik maga"]},{"lvl":60,"vendor":"Kowal Unil","items":["Język żaru i ognia","Tarcza nocy i dni"]},{"lvl":65,"vendor":"Kowal Unil","items":["Odzienie znawcy zaklęć"]},{"lvl":65,"vendor":"Sprzedawca Roan","items":["Krwawe pięści maga","Kask pogromcy gnolli","Szczecina gnolla","Kryształowe iskry","Chabrowa zawieszka"]},{"lvl":70,"vendor":"Kowal Unil","items":["Przestroga żywiołaka","Blokada herosa"]},{"lvl":75,"vendor":"Kowal Unil","items":["Skórzany strój herosa"]},{"lvl":75,"vendor":"Sprzedawca Roan","items":["Splot ametystowych nici","Srebrna kopuła śmiałka","Cholewy czarnoksiężnika","Splot ametystowych nici","Pycha Wernoradu","Odurzająca woń hortensji"]},{"lvl":80,"vendor":"Szybki Daraker","items":["Płomienny sejmitar","Ochrona gadziego odkrywcy"]},{"lvl":85,"vendor":"Handlarka Halidura","items":["Zamszowy chwyt gadziny","Hełm leśnego licha","Smocze podeszwy","Zamszowy chwyt gadziny","Iglasty kryształ","Gadzia kolia"]},{"lvl":85,"vendor":"Szybki Daraker","items":["Kolczuga smoczej łuski"]},{"lvl":90,"vendor":"Szybki Daraker","items":["Wyszczerbione ostrze pożogi","Tarcza z fiolką antidotum"]},{"lvl":95,"vendor":"Handlarka Halidura","items":["Wytwór syczącego rzemieślnika","Okazałe rogi centaura","Mytharskie cholewy","Wytwór syczącego rzemieślnika","Rubinowy język kameleona","Smoczy wisior"]},{"lvl":95,"vendor":"Szybki Daraker","items":["Puszka doświadczonego paladyna"]},{"lvl":100,"vendor":"Szybki Daraker","items":["Stal rozgrzana smoczym oddechem","Ochrona smoczych pazurów"]}],"Tancerz ostrzy":[{"lvl":10,"vendor":"Ernest","items":["Rdzewiejący kask","Obuwie do tańca","Zrabowany pierścionek","Uwierający kamyk"]},{"lvl":10,"vendor":"Skrzynie","items":["Wietrzny rozplatacz warkoczy","Mysi pocałunek","Kubrak nadzorcy taboru"]},{"lvl":16,"vendor":"Ernest","items":["Przyłbica przyszłego bochatera","Porzucone mienie","Niskiej wartości pamiątka rodzinna","Wróżbiarska gwiazdka"]},{"lvl":16,"vendor":"Skrzynie","items":["Pałasz Szagriatów","Szabelka spod szyldu Amore","Kapota doli i niedoli"]},{"lvl":20,"vendor":"Kowal Alrik","items":["Lekki pancerz skrytobójcy","Lekkie ostrze tancerza","Sztylet młodego rzezimieszka"]},{"lvl":20,"vendor":"Umbar","items":["Pospolite trepy rycerza","Kask myśliwego","Pierścień sprawności","Amulet rycerza"]},{"lvl":25,"vendor":"Kowal Alrik","items":["Pogromca szkieletów","Złota rysa"]},{"lvl":28,"vendor":"Umbar","items":["Chwyt żmii","Stalowa czapka","Wzmocnione podeszwy","Oko fałszu","Malachitowa zawieszka"]},{"lvl":30,"vendor":"Kowal Alrik","items":["Czarnoostrze","Kordzik skrytobójcy"]},{"lvl":36,"vendor":"Anaret","items":["Bitewny diabeł","Goblinie cichobiegi","Czaszka pychy","Szczęście podróżnika"]},{"lvl":36,"vendor":"Szmugler Beniamin","items":["Mroczny kordelas","Podręczne ostrze strażnika"]},{"lvl":41,"vendor":"Anaret","items":["Ochrona wilczych skór","Skórzany hełm leszego"]},{"lvl":41,"vendor":"Szmugler Beniamin","items":["Miecz sprawiedliwości","Zniszczona pamiątka","Skórzana kamizela tancerza"]},{"lvl":46,"vendor":"Anaret","items":["Czarna zguba bojownika","Szkiełko w srebrnej oprawie"]},{"lvl":46,"vendor":"Szmugler Beniamin","items":["Przeklęta broń gwardzisty","Sztylet wspinaczkowy"]},{"lvl":48,"vendor":"Anaret","items":["Rękawice z mokradeł","Słoneczne frędzle"]},{"lvl":48,"vendor":"Szmugler Beniamin","items":["Kolczuga skrytobójcy amatora"]},{"lvl":50,"vendor":"Kowal Unil","items":["Pałasz honoru i chwały","Sztylet okrętowy"]},{"lvl":55,"vendor":"Kowal Unil","items":["Strój ze znakiem śmierci"]},{"lvl":55,"vendor":"Sprzedawca Roan","items":["Poplamione rękawice strażnika","Osłona młodej harpii","Szelest nieuważnego kobolda","Odznaka młodego asasyna","Zielony wabiciel żądłaków"]},{"lvl":60,"vendor":"Kowal Unil","items":["Ekwipunek szlachcica","Duma rzeźnika"]},{"lvl":65,"vendor":"Kowal Unil","items":["Klata pulsujących świateł"]},{"lvl":65,"vendor":"Sprzedawca Roan","items":["Lniane otuldłonie","Czapka łowcy tygrysów","Szczecina gnolla","Symbol spopielenia","Kwiecisty popis jubilera"]},{"lvl":70,"vendor":"Kowal Unil","items":["Szeroki metal strażnika","Stalowy płatek"]},{"lvl":75,"vendor":"Kowal Unil","items":["Pancerz zwycięzcy turnieju"]},{"lvl":75,"vendor":"Sprzedawca Roan","items":["Zakurzone wojenne rękawice","Srebrna kopuła śmiałka","Pozłacana duma rycerza","Zakurzone wojenne rękawice","Koralowa narośl","Wonne pąki kniei"]},{"lvl":80,"vendor":"Szybki Daraker","items":["Podręczna mytharska maczeta","Ostrze grzechotnika"]},{"lvl":85,"vendor":"Handlarka Halidura","items":["Zamszowy chwyt gadziny","Prosta wężowa łebka","Ochrona przed igliwiem","Zamszowy chwyt gadziny","Bagienna narośl","Naszyjnik ofiary bagien"]},{"lvl":85,"vendor":"Szybki Daraker","items":["Blaszane oko bogów"]},{"lvl":90,"vendor":"Szybki Daraker","items":["Piła tnąca konary","Żelazna strzałka"]},{"lvl":95,"vendor":"Handlarka Halidura","items":["Ochrona z wężowej skóry","Świerkowy kask","Mytharskie cholewy","Ochrona z wężowej skóry","Koralowy język żaby","Ozdoba wężowego kochanka"]},{"lvl":95,"vendor":"Szybki Daraker","items":["Zbroja doświadczonego tancerza"]},{"lvl":100,"vendor":"Szybki Daraker","items":["Wężowy urwiłeb","Haratanie wężowego ogona"]}],"Tropiciel":[{"lvl":10,"vendor":"Ernest","items":["Rdzewiejący kask","Obuwie do tańca","Zrabowany pierścionek","Uwierający kamyk"]},{"lvl":10,"vendor":"Skrzynie","items":["Łuk cienia i blasku","Mysie kości","Stare barwione płótno"]},{"lvl":16,"vendor":"Ernest","items":["Przyłbica przyszłego bochatera","Porzucone mienie","Niskiej wartości pamiątka rodzinna","Wróżbiarska gwiazdka"]},{"lvl":16,"vendor":"Skrzynie","items":["Celownik Muriatów","Pociski z więdnących liści","Peleryna wiernego kompana"]},{"lvl":20,"vendor":"Szagarat Czarny Łowca","items":["Słoneczny kamuflaż","Płomienna cięciwa","Iskrzące strzały bukowe"]},{"lvl":20,"vendor":"Umbar","items":["Pospolite trepy rycerza","Hełm maga bojowego","Pierścień sprawności","Magiczna błyskotka"]},{"lvl":25,"vendor":"Szagarat Czarny Łowca","items":["Łuk dotkliwych poparzeń","Iskrzące strzały klonowe"]},{"lvl":28,"vendor":"Umbar","items":["Eteryczny dotyk","Stalowa czapka","Magiczne trzewiki","Ziarno prawdy","Medalion z turkusem"]},{"lvl":30,"vendor":"Szagarat Czarny Łowca","items":["Ogniste skrzydła","Iskrzące strzały wiązowe"]},{"lvl":36,"vendor":"Anaret","items":["Wzmocniona ochrona maga","Obuwie z magiczną powłoką","Czaszka gniewu","Los tułacza"]},{"lvl":36,"vendor":"Mroczny Zgrzyt","items":["Objęcie młodego smoka","Gorące strzały zbója"]},{"lvl":41,"vendor":"Anaret","items":["Magiczny dotyk rabusia","Wizja trzeciego oka"]},{"lvl":41,"vendor":"Mroczny Zgrzyt","items":["Piekielna tortura","Gorące strzały bandziora","Płaszcz trapera"]},{"lvl":46,"vendor":"Anaret","items":["Czarna zguba bojownika","Gwiazda nadziei"]},{"lvl":46,"vendor":"Mroczny Zgrzyt","items":["Manotrówczy naciąg tropiciela","Gorące strzały rzezimieszka"]},{"lvl":48,"vendor":"Anaret","items":["Rękawice z mokradeł"]},{"lvl":48,"vendor":"Mroczny Zgrzyt","items":["Złota pierś myśliwego"]},{"lvl":50,"vendor":"Huslin","items":["Skwierczący naciąg","Kompletny kołczan tropiciela"]},{"lvl":55,"vendor":"Huslin","items":["Odzienie złotych pyłków"]},{"lvl":55,"vendor":"Sprzedawca Roan","items":["Poplamione rękawice strażnika","Hełm łowcy koboldów","Obuwie pokryte pajęczyną","Pierścień ulotnej myśli","Miododajny naszyjnik maga"]},{"lvl":60,"vendor":"Huslin","items":["Ogniste szczypce","Doskonały kołczan tropiciela"]},{"lvl":65,"vendor":"Huslin","items":["Płaszcz tropiciela z wyżyn"]},{"lvl":65,"vendor":"Sprzedawca Roan","items":["Krwawe pięści maga","Magiczne wsparcie demiliszy","Szczecina gnolla","Kryształowe iskry","Medalion fotosyntezy"]},{"lvl":70,"vendor":"Huslin","items":["Soczysty łuk anemonów","Mistrzowski kołczan tropiciela"]},{"lvl":75,"vendor":"Huslin","items":["Opieka leśnej istoty"]},{"lvl":75,"vendor":"Sprzedawca Roan","items":["Zemsta banity","Srebrna kopuła śmiałka","Cholewy czarnoksiężnika","Zemsta banity","Sygnet wyniosłości","Odurzająca woń hortensji"]},{"lvl":80,"vendor":"Tropicielka Olekusa","items":["Wyrzutnia rozgotowanych żab","Płonące strzały na ropuchy"]},{"lvl":85,"vendor":"Handlarka Halidura","items":["Zamszowy chwyt gadziny","Hełm leśnego licha","Fikuśne kamasze maga","Zamszowy chwyt gadziny","Iglasty kryształ","Amulet przemiany w gada"]},{"lvl":85,"vendor":"Tropicielka Olekusa","items":["Zbożowy kamuflaż węża"]},{"lvl":90,"vendor":"Tropicielka Olekusa","items":["Gniewna cięciwa z Mythar","Płonące strzały na węże"]},{"lvl":95,"vendor":"Handlarka Halidura","items":["Wytwór syczącego rzemieślnika","Podarunek Introprodara","Mytharskie cholewy","Wytwór syczącego rzemieślnika","Jaspisowy język węża","Smoczy wisior"]},{"lvl":95,"vendor":"Tropicielka Olekusa","items":["Odzienie doświadczonego tropiciela"]},{"lvl":100,"vendor":"Tropicielka Olekusa","items":["Zew Introprodara","Płonące strzały na aligatory"]}],"Wojownik":[{"lvl":10,"vendor":"Ernest","items":["Rdzewiejący kask","Obuwie do tańca","Zrabowany pierścionek","Uwierający kamyk"]},{"lvl":10,"vendor":"Skrzynie","items":["Obrońca kolorowych wozów","Osłona kłamliwego wróżbiarza","Pancerz w sam raz"]},{"lvl":16,"vendor":"Ernest","items":["Przyłbica przyszłego bochatera","Porzucone mienie","Niskiej wartości pamiątka rodzinna","Wróżbiarska gwiazdka"]},{"lvl":16,"vendor":"Skrzynie","items":["Topór rodziny Szabadabada","Dawczyni radości","Blachy milczącego świata"]},{"lvl":20,"vendor":"Kowal Alrik","items":["Czarna kamizela","Uniwersalne ostrze","Tarcza początkującego"]},{"lvl":20,"vendor":"Umbar","items":["Pospolite trepy rycerza","Rogi odkrywcy","Ozdoba wojaka","Amulet rycerza"]},{"lvl":25,"vendor":"Kowal Alrik","items":["Kątownik","Lepsza ochrona wojaka"]},{"lvl":28,"vendor":"Umbar","items":["Kamienny uścisk","Stalowa czapka","Wzmocnione podeszwy","Oko fałszu","Kwiecie ametystu"]},{"lvl":30,"vendor":"Kowal Alrik","items":["Smukły miecz rycerza","Rogi bawoła"]},{"lvl":36,"vendor":"Anaret","items":["Bitewny diabeł","Kamasze odważnego rycerza","Czaszka pychy","Szczęście podróżnika"]},{"lvl":36,"vendor":"Szmugler Beniamin","items":["Falchion podróżnika","Tarcza obita skórą"]},{"lvl":41,"vendor":"Anaret","items":["Ochrona wilczych skór","Hełm byczej siły"]},{"lvl":41,"vendor":"Szmugler Beniamin","items":["Kliga ostrych zębów","Zakrwawiona tafla","Bojowy serdak"]},{"lvl":46,"vendor":"Anaret","items":["Czarna zguba bojownika","Szkiełko w srebrnej oprawie"]},{"lvl":46,"vendor":"Szmugler Beniamin","items":["Ostrze dekapitacji","Zderzak"]},{"lvl":48,"vendor":"Anaret","items":["Popis garbarza"]},{"lvl":48,"vendor":"Szmugler Beniamin","items":["Odzienie ze skóry dzika"]},{"lvl":50,"vendor":"Kowal Unil","items":["Eleganckie ostrze wojownika","Kolczasta osłona"]},{"lvl":55,"vendor":"Kowal Unil","items":["Wytwór miejscowej szwaczki"]},{"lvl":55,"vendor":"Sprzedawca Roan","items":["Poplamione rękawice strażnika","Osłona młodej harpii","Szara piechota","Odwet wojownika","Zielony wabiciel żądłaków"]},{"lvl":60,"vendor":"Kowal Unil","items":["Stalowy zawijas","Tarcza dawnej chwały"]},{"lvl":65,"vendor":"Kowal Unil","items":["Wzmocniona ochrona żeber"]},{"lvl":65,"vendor":"Sprzedawca Roan","items":["Lniane otuldłonie","Kask pogromcy gnolli","Szczecina gnolla","Symbol spopielenia","Kwiecisty popis jubilera"]},{"lvl":70,"vendor":"Kowal Unil","items":["Wzmocniona ochrona żeber","Patronat demona"]},{"lvl":75,"vendor":"Kowal Unil","items":["Wytworna kamizela wojownika"]},{"lvl":75,"vendor":"Sprzedawca Roan","items":["Zakurzone wojenne rękawice","Srebrna kopuła śmiałka","Pozłacana duma rycerza","Zakurzone wojenne rękawice","Pycha Wernoradu","Wonne pąki kniei"]},{"lvl":80,"vendor":"Szybki Daraker","items":["Falchion żabiej czaszki","Iglasta osłona"]},{"lvl":85,"vendor":"Handlarka Halidura","items":["Zamszowy chwyt gadziny","Prosta wężowa łebka","Smocze podeszwy","Zamszowy chwyt gadziny","Bagienna narośl","Gadzia kolia"]},{"lvl":85,"vendor":"Szybki Daraker","items":["Filcowa opieka gadów"]},{"lvl":90,"vendor":"Szybki Daraker","items":["Czarna strzała wojownika","Tarcza połowicznej przemiany"]},{"lvl":95,"vendor":"Handlarka Halidura","items":["Ochrona z wężowej skóry","Okazałe rogi centaura","Mytharskie cholewy","Ochrona z wężowej skóry","Rubinowy język kameleona","Ozdoba wężowego kochanka"]},{"lvl":95,"vendor":"Szybki Daraker","items":["Puszka doświadczonego paladyna"]},{"lvl":100,"vendor":"Szybki Daraker","items":["Cięcie biesa","Trofeum centaura"]}],"Łowca":[{"lvl":10,"vendor":"Ernest","items":["Rdzewiejący kask","Obuwie do tańca","Zrabowany pierścionek","Uwierający kamyk"]},{"lvl":10,"vendor":"Skrzynie","items":["Kusza zamkniętych oczu","Mysie kości","Odzienie spadających liści"]},{"lvl":16,"vendor":"Ernest","items":["Przyłbica przyszłego bochatera","Porzucone mienie","Niskiej wartości pamiątka rodzinna","Wróżbiarska gwiazdka"]},{"lvl":16,"vendor":"Skrzynie","items":["Trojka","Pociski z więdnących liści","Kaftan dzikich trefli"]},{"lvl":20,"vendor":"Szagarat Czarny Łowca","items":["Czarny płaszcz ucznia","Wzmocniony naciąg","Krótkie strzały bukowe"]},{"lvl":20,"vendor":"Umbar","items":["Pospolite trepy rycerza","Kask myśliwego","Pierścień sprawności","Amulet rycerza"]},{"lvl":25,"vendor":"Szagarat Czarny Łowca","items":["Wyrzutnia ostrych strzał","Krótkie strzały klonowe"]},{"lvl":28,"vendor":"Umbar","items":["Chwyt żmii","Stalowa czapka","Wzmocnione podeszwy","Oko fałszu","Malachitowa zawieszka"]},{"lvl":30,"vendor":"Szagarat Czarny Łowca","items":["Drżąca cięciwa","Krótkie strzały wiązowe"]},{"lvl":36,"vendor":"Anaret","items":["Bitewny diabeł","Goblinie cichobiegi","Czaszka gniewu","Szczęście podróżnika"]},{"lvl":36,"vendor":"Mroczny Zgrzyt","items":["Samoobrona łowcy","Ostre strzały zbója"]},{"lvl":41,"vendor":"Anaret","items":["Ochrona wilczych skór","Skórzany hełm leszego"]},{"lvl":41,"vendor":"Mroczny Zgrzyt","items":["Zdobiony łuk odkrywcy","Ostre strzały bandziora","Krucze odzienie"]},{"lvl":46,"vendor":"Anaret","items":["Czarna zguba bojownika","Szkiełko w srebrnej oprawie"]},{"lvl":46,"vendor":"Mroczny Zgrzyt","items":["Naciąg szaleństwa","Ostre strzały rzezimieszka"]},{"lvl":48,"vendor":"Anaret","items":["Rękawice z mokradeł"]},{"lvl":48,"vendor":"Mroczny Zgrzyt","items":["Leśny kaftan trapera"]},{"lvl":50,"vendor":"Huslin","items":["Broń doświadczonego łowcy","Kompletny kołczan łowcy"]},{"lvl":55,"vendor":"Huslin","items":["Skórzana kurta łowcy"]},{"lvl":55,"vendor":"Sprzedawca Roan","items":["Poplamione rękawice strażnika","Osłona młodej harpii","Szelest nieuważnego kobolda","Odznaka młodego asasyna","Zielony wabiciel żądłaków"]},{"lvl":60,"vendor":"Huslin","items":["Przekleństwo leśnej zwierzyny","Doskonały kołczan łowcy"]},{"lvl":65,"vendor":"Huslin","items":["Karmazynowy lekki pancerz"]},{"lvl":65,"vendor":"Sprzedawca Roan","items":["Lniane otuldłonie","Czapka łowcy tygrysów","Szczecina gnolla","Symbol spopielenia","Medalion fotosyntezy"]},{"lvl":70,"vendor":"Huslin","items":["Szybkostrzelik myśliwego","Mistrzowski kołczan łowcy"]},{"lvl":75,"vendor":"Huslin","items":["Warstwa czarnej szczeciny"]},{"lvl":75,"vendor":"Sprzedawca Roan","items":["Zemsta banity","Srebrna kopuła śmiałka","Pozłacana duma rycerza","Zemsta banity","Koralowa narośl","Wonne pąki kniei"]},{"lvl":80,"vendor":"Tropicielka Olekusa","items":["Naciąg łowcy węży","Ostre strzały na ropuchy"]},{"lvl":85,"vendor":"Handlarka Halidura","items":["Zamszowy chwyt gadziny","Prosta wężowa łebka","Ochrona przed igliwiem","Zamszowy chwyt gadziny","Bagienna narośl","Naszyjnik ofiary bagien"]},{"lvl":85,"vendor":"Tropicielka Olekusa","items":["Ukrycie czarnej kobry"]},{"lvl":90,"vendor":"Tropicielka Olekusa","items":["Cięciwa orlich piór","Ostre strzały na węże"]},{"lvl":95,"vendor":"Handlarka Halidura","items":["Ochrona z wężowej skóry","Świerkowy kask","Mytharskie cholewy","Ochrona z wężowej skóry","Koralowy język żaby","Ozdoba wężowego kochanka"]},{"lvl":95,"vendor":"Tropicielka Olekusa","items":["Kurta doświadczonego łowcy"]},{"lvl":100,"vendor":"Tropicielka Olekusa","items":["Pleciona wyrzutnia konarów","Ostre strzały na aligatory"]}]};
    ...
    (function(){var o=document.createElement('option'); o.value='zatopiony-szczyt-skrzynie'; o.textContent='Zatopiony Szczyt – Skrzynie'; equipSel.appendChild(o);})();
    (function(){var o=document.createElement('option'); o.value='zatopiony-szczyt-ernest'; o.textContent='Zatopiony Szczyt – Ernest'; equipSel.appendChild(o);})();
...
// ===== AUTO AUCTION WALKER (fixed: packet-driven + no-tab dependency) =====
(function(){
  const CHECK_MS = 1800;
  const START_COOLDOWN_MS = 15000;
  let lastAutoAuctionAt = 0;
  let lastKnownBag = null;

  function adiIsBotEnabledNow(){
    try{
      if(localStorage.getItem('adi-bot_enabled') === '1') return true;
    }catch(_){ }
    try{
      return !!(window.adiwilkTestBot && window.parseInput === window.adiwilkTestBot.botPI);
    }catch(_){ }
    return false;
  }

  function adiLoadAuctionCfgSafe(){
    try{
      if(typeof adiLoadAuctionCfg === 'function'){
  ...
  function adiParseBagText(txt){
    try{
      const s = String(txt || '').trim();
      if(!s) return null;
      const m1 = s.match(/wolnych miejsc w torbach:\s*(\d+)/i);
      if(m1) return { free: Math.max(0, Number(m1[1] || 0)), source: 'auction-panel-text' };
      const m2 = s.match(/(\d+)\s*\/\s*(\d+)/);
      if(m2){
        return {
          free: Math.max(0, Number(m2[1] || 0)),
          total: Math.max(0, Number(m2[2] || 0)),
          used: Math.max(0, Number(m2[2] || 0) - Number(m2[1] || 0)),
          source: 'bag-counter-text'
        };
      }
    }catch(_){ }
    return null;
  ...
  function adiSafeBagSpaceCount(){
    try{
      if(typeof window.__adiAuctionUpdateBagSpace === 'function'){
        try{ window.__adiAuctionUpdateBagSpace(); }catch(_){ }
      }
    }catch(_){ }
      ...
      if(typeof adiGetTotalBagSpace === 'function'){
        ...
        if(res && Number.isFinite(Number(res.free))){
          lastKnownBag = {
            ...
            used: Number.isFinite(Number(res.used)) ? Math.max(0, Number(res.used)) : null,
    ...
    try{
      ...
      if(out){
        const parsed = adiParseBagText(out.textContent || out.innerText || '');
        if(parsed){
          lastKnownBag = parsed;
          return lastKnownBag;
        }
    ...
    try{
      ...
      for(const id of ['bs0','bs1','bs2']){
        ...
        if(!el) continue;
        const parsed = adiParseBagText(el.textContent || el.innerText || '');
        if(parsed && Number.isFinite(Number(parsed.free))){
          if(!agg) agg = { free: 0, total: 0, used: 0, source: 'bs-text' };
  ...
  function adiCanStartAutoAuction(){
    try{
      if(!adiIsBotEnabledNow()) return false;
  ...
  function adiCheckAutoAuction(source){
    try{
      ...
      if(Number(bagSpace.free) > threshold) return false;

      return !!window.__adiStartAuctionWalk('auto-free<=' + threshold + ':' + String(source || 'timer'));
    }catch(_){ }
    return false;
  }
  ...
  window.__adiCheckAutoAuctionNow = function(source){
    return adiCheckAutoAuction(source || 'manual-debug');
  };

  setInterval(function(){
    try{ adiCheckAutoAuction('timer'); }catch(_){ }
  }, CHECK_MS);

  function adiInstallAuctionParseHooks(){
    try{
      const bot = window.adiwilkTestBot;
      if(bot){
        if(typeof bot.basePI === 'function' && !bot.basePI.__adiAuctionWrapped){
          const old = bot.basePI;
          bot.basePI = function(){
            const ret = old.apply(this, arguments);
            try{ setTimeout(function(){ adiCheckAutoAuction('basePI'); }, 50); }catch(_){ }
            return ret;
          };
          bot.basePI.__adiAuctionWrapped = true;
        }
        if(typeof bot.botPI === 'function' && !bot.botPI.__adiAuctionWrapped){
          const old = bot.botPI;
          bot.botPI = function(){
            const ret = old.apply(this, arguments);
            try{ setTimeout(function(){ adiCheckAutoAuction('botPI'); }, 50); }catch(_){ }
            return ret;
          };
          bot.botPI.__adiAuctionWrapped = true;
        }
      }

      if(typeof window.parseInput === 'function' && !window.parseInput.__adiAuctionWrapped){
        const old = window.parseInput;
        window.parseInput = function(){
          const ret = old.apply(this, arguments);
          try{ setTimeout(function(){ adiCheckAutoAuction('parseInput'); }, 50); }catch(_){ }
          return ret;
        };
        window.parseInput.__adiAuctionWrapped = true;
      }
    }catch(_){ }
  }

  adiInstallAuctionParseHooks();
  setTimeout(adiInstallAuctionParseHooks, 1500);
  setTimeout(adiInstallAuctionParseHooks, 5000);
})();
...
(function(){
  ...
  function adiEnsureAuctionTab(){
    try{
      ...
      window.__adiAuctionUpdateBagSpace = updateBagSpace;
      updateBagSpace();
      try{
